...
export const chat = async (req, res) => {
  ...
  const answers = matchedItems
    .map(item => item.answer)
    .filter(Boolean); // remove any undefined or null answers
  ...
  return res.status(200).json({
    ...
    if (fuzzyResults.length > 0) {
      return res.status(200).json({ status: true, answer: fuzzyResults[0].item.answer });
    }
...
export const chatrsidebar = async(req,res) =>{
    try {
    const { id, source } = req.query;
    if(source==='advisor'){
      const chatRooms = await ChatRoom.find({ advisorId:id });

      const sidebarList = await Promise.all(chatRooms.map(async (room) => {
...
export const getMessagesByRoom = async (req, res) => {
  try {
    ...
    const messages = await Message.find({ chatRoomId }).sort({ sentAt: 1 }); // oldest to newest
...
export const sendMessage = async (req, res) => {
  try {
    ...
    const message = await Message.create({ chatRoomId, senderId, receiverId, text });
